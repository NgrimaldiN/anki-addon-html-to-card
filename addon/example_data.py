...
LLM_GENERATION_GUIDANCE = (
    ...
    "in Anki Add Cards. If you omit note_type, first select Anki's working 'Basic' "
    "note type first, or another known-good note type, before importing. Keep the "
    "content compatible with Anki's stock Front/Back behavior and minimal styling "
    "unless the user explicitly asks for more design. If the user wants something "
    "more distinctive, you may return fully custom HTML/CSS, additional fields, "
    "richer layouts, and more original visual design. Only include note_type when "
    "custom fields, templates, or CSS are actually needed. Return only one JSON "
    "object, with no trailing semicolon, no extra wrapper braces, and no second "
    "object after it. CRITICAL JSON FORMATTING RULES: never place literal physical "
    "newlines, tabs, or other control characters inside JSON string values; keep "
    "each JSON string on one physical line and use escaped sequences like \\n or \\t "
    "when you need formatting. Escape every literal backslash inside JSON strings; "
    "for math or LaTeX-style commands, write \\\\gamma, \\\\pi, \\\\epsilon, etc., "
    "or prefer Unicode symbols like γ, π, ε."
)
